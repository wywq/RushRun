<template>
	<div>
		<el-card >
			<el-button type="success" size="small" @click="add">添加分类</el-button>
		</el-card >
		<el-card >
			...
			<el-table :data="list" border style="width: 100%">
					...
					<template slot-scope="props">
						<el-table :data="props.row.children" border style="width: 100%">
							...
							<el-table-column prop="status" label="状态" width="160px">
								<template slot-scope="scope">
									<el-switch v-model="scope.row.status" @change="updatestatus(scope.row.id)" active-text="开启" inactive-text="关闭" :active-value="1">
									</el-switch>
								</template>
							</el-table-column>
							<el-table-column  label="操作">
								<template slot-scope="scope">
									<el-button @click="bian(scope.row,2)" type="primary" size="mini">编辑</el-button>
				...
				<el-table-column  label="操作">
					<template slot-scope="scope">
						<el-button @click="bian(scope.row,1)" type="primary" size="mini">编辑</el-button>
						<el-button @click="del(scope.row.id)" type="danger" size="mini">删除</el-button>
					</template>
				</el-table-column>
			  ...
			  <div class="demo-drawer__content">
			    <el-form :model="addform" label-width="100px">
					<!-- <el-form-item label="店铺">
						<el-select v-model="addform.shop_id" placeholder="请选择" 
						:disabled="id?true:false"
						 size="mini" @change="addform.pid='',getfenleilist()">
					...
					<el-form-item label="分类等级">
						<el-radio v-model="addform.isp" label="1" border size="mini"  :disabled="id?true:false">一级分类</el-radio>
						 <el-radio v-model="addform.isp" label="2" border size="mini" :disabled="id?true:false">二级分类</el-radio>
					</el-form-item>
...
<script>
	...
	export default {
		...
		methods: {
			...
			//删除
			async del(id){
				let res = await this.$zcy.delFenlei({id})
				if(res.status = 1){
