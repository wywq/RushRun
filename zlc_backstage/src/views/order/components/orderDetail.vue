			...
			<el-row>
				<el-col :span="6">
					<el-card>
						...
						<div class="item1">
							...
							<div class="cont1">普通订单</div>
							<!-- <div class="cont1">{{orderDetail.orderInfo.ord_type==1?'普通订单':orderDetail.orderInfo.ord_type==2?'秒杀订单':'拼团订单'}}</div> -->
						</div>
						<!-- <div class="item1">
							<div class="lable1">付款方式：</div>
							<div class="cont1" v-if="orderDetail.orderInfo.type == 0">微信支付</div>
							<div class="cont1" v-if="orderDetail.orderInfo.type == 1">支付宝支付</div>
							<div class="cont1" v-if="orderDetail.orderInfo.type == 2">酷豆支付</div>
							<div class="cont1" v-if="orderDetail.orderInfo.type == 3">贡献值支付</div>
						</div> -->
						<div class="item1">
							<div class="lable1">买家：</div>
							<div class="cont1">{{orderDetail.memberInfo.nickname}}</div>
						</div>
						<div class="item1">
				...
				<el-col :span="18">
					<el-card>
						...
						<div>
							<!-- <el-button size="mini" type="primary" v-if="orderDetail.orderInfo.status>=3" @click="$emit('seeping')">查看评价</el-button> -->
							<el-button size="mini" type="primary" v-if="orderDetail.orderInfo.status==1" @click="$emit('fahuo')">发货</el-button>
							<el-button size="mini" type="primary" v-if="orderDetail.orderInfo.status>=2" @click="$emit('wuliu',{oid,shipping_code:orderDetail.post.shipping_code,num:orderDetail.orderInfo.post_number})">查看物流</el-button>
