			...
			<el-table :data="list" border style="width: 100%">
				...
				<el-table-column prop="logo" label="店铺logo">
					<template slot-scope="scope">
						<img :src="scope.row.shopLogo" class="logo-icon" />
					</template>
				</el-table-column>
				<el-table-column prop="name" label="姓名"></el-table-column>
				<el-table-column prop="phone" label="电话"></el-table-column>
				<!-- <el-table-column prop="IDcard" label="身份证号"></el-table-column> -->
				<el-table-column prop="shopName" label="店铺名" show-overflow-tooltip></el-table-column>
				<el-table-column prop="shop_address" label="地址" show-overflow-tooltip></el-table-column>
				<el-table-column prop="shopStatus" label="店铺状态">
					<template slot-scope="scope">
						<el-switch :value="scope.row.is_lock==0" @change="updateShopStatus(scope.row.shop_id)">
						</el-switch>
					</template>
				</el-table-column>
				<el-table-column prop="shopStatus" label="店铺状态">
					<template slot-scope="scope">
						<div v-if="scope.row.shopStatus == 0">已注册</div>
						<div v-else-if="scope.row.shopStatus == 1">申请中</div>
						<div v-else-if="scope.row.shopStatus == 2">审核通过</div>
						<div v-else-if="scope.row.shopStatus == 3">禁用中</div>
					</template>
				</el-table-column>
				...
				<el-table-column prop="star" label="店铺等级" fixed="right" width="165px">
					<template slot-scope="scope">
						<el-rate v-model="scope.row.star" disabled show-score text-color="#ff9900">
						</el-rate>
					</template>
				</el-table-column>
				<el-table-column prop="" label="操作" fixed="right" width="140px" >
					<template slot-scope="scope">
						<el-dropdown trigger="click" split-button size="mini" type="primary">
							...
							<el-dropdown-menu slot="dropdown">
								<!-- <el-dropdown-item>
									<el-button type="success" size="mini" @click="$router.push('/goodstype?shop_id=' +scope.row.shop_id)" style="margin:5px 0">分类</el-button>
								</el-dropdown-item> -->
								<el-dropdown-item>
									<el-button type="primary" size="mini" @click="modify(scope.row.shop_id,scope.row)" style="margin:5px 0">修改</el-button>
								</el-dropdown-item>
								<el-dropdown-item>
									<el-button size="mini" @click="detail(scope.row.shop_id)" style="margin:5px 0">详情</el-button>
								</el-dropdown-item>
							</el-dropdown-menu>
						</el-dropdown>
