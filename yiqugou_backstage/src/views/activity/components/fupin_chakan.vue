		...
		<el-card>
			<div style="width: 100%">
				<el-table :data="list" border style="width: 100%">
					<el-table-column prop="list_id" label="预约ID">
					</el-table-column>
					<el-table-column prop="member_id" label="预约用户ID">
					</el-table-column>
					<el-table-column prop="member_name" label="预约用户昵称">
					</el-table-column>
					<el-table-column prop="money" label="预约保证金">
					</el-table-column>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async auctionYyue(){
				let res = await this.$zcy.auctionYyue({level_id:this.id})
				if (res.status == 1) {
					this.list =res.data
				}else{
