  ...
  <div>
    ...
    <el-card>
      <el-table :data="list" border style="width: 100%">
        ...
        <el-table-column prop="member.nick" label="评价用户昵称"></el-table-column>
        <el-table-column prop="member.name" label="评论人姓名"></el-table-column>
        <el-table-column prop="goods.pic" label="评论图片">
          <template slot-scope="scope">
            <el-image class="goods-pic" :src="scope.row.pic[0]" :preview-src-list="scope.row.pic"></el-image>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    async getlist() {
      ...
      let res = await this.$zcy.commentList(postdata);
    ...
    async delComment(comment_id) {
      this.$confirm("确认删除？")
        .then(async () => {
			console.log(123)
          let res = await this.$zcy.commentDel({ comment_id });
          if (res.status == 1) {
