<template>
	<div>
		...
		<el-card v-if="!showlog">
			<el-form :inline="true" ref="form" :model="form" class="demo-form-inline">
				...
				<!-- <el-form-item label="账号">
					<el-input v-model="form.phone" placeholder="按账号搜索" clearable size="mini"></el-input>
				</el-form-item> -->
				<el-form-item label="用户昵称">
					<el-input v-model="form.nick" placeholder="按昵称搜索" clearable size="mini"></el-input>
				</el-form-item>
				<!-- </br> -->
						...
						<div class="goods_item" v-for="(item,i) in scope.row.info" :key="i" :class="i==scope.row.info.length-1?'border0':''">
							...
							<div class="cont_ite">
								...
								<div class="lable">
									...
									<span>{{item.price}}{{item.money_type}} x{{item.num}}件 </span>
								</div>
							</div>
						</div>
					</template>
						...
						<el-dropdown trigger="click" split-button size="mini" type="primary">
							...
							<el-dropdown-menu slot="dropdown">
								...
								<!-- <el-dropdown-item v-if="scope.row.status>=4">
									<el-button type="warning" size="mini" @click="id=scope.row.order_id,issee= true" style="margin: 5px 0;">查看评价</el-button>
								</el-dropdown-item> -->
								...
								<!-- <el-dropdown-item v-if="scope.row.status>=2" style="text-align: center;">
									<el-button size="mini" @click="id=scope.row.order_id,iswuliu=true" style="margin: 5px 0;">查看物流</el-button>
								</el-dropdown-item> -->
								<el-dropdown-item v-if="scope.row.status>=2" style="text-align: center;">
									<el-button size="mini" @click="id=scope.row.order_id,showPostDialog= true" style="margin: 5px 0;">修改物流</el-button>
								</el-dropdown-item>
								<el-dropdown-item v-if="scope.row.status==1" style="text-align: center;">
									<el-button size="mini" type="warning" @click="openaddress(scope.row)" style="margin: 5px 0;">修改地址</el-button>
								</el-dropdown-item>
								<!-- <el-dropdown-item v-if="scope.row.status==1" style="text-align: center;">
									<el-button size="mini" type="danger" @click="tuikuan(scope.row.info)" style="margin: 5px 0;">退款</el-button>
								</el-dropdown-item> -->
							</el-dropdown-menu>
						</el-dropdown>
		...
		<!-- 物流 -->
		<el-drawer title="物流信息" :visible.sync="iswuliu" direction="rtl" size="50%">
			<wuliu :order_id="id"  :num="num" :shipping_code="shipping_code" v-if="iswuliu"></wuliu>
		</el-drawer>
...
<script>
	...
	export default {
		...
		methods: {
			...
			wuliu(o){
				console.log(o,1111111111)
				this.id=o.oid
				this.shipping_code=o.shipping_code
				this.num = o.num
				this.iswuliu=true
			},
			//获取订单类型
