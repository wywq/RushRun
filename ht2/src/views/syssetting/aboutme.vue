...
<script>
	...
	export default {
		...
		methods: {
			async xianshi(){
				const aboutMe = await service.aboutMe();
				if (aboutMe.data.status == 1) {
					this.article.contentHtml = aboutMe.data.data
					this.$message.success(aboutMe.data.info);
				}else{
					this.$message.error(aboutMe.data.info);
				}
			...
			async submitArticle() {
				const updateAbout = await service.updateAbout({
					content:this.editor.txt.html()
				})
				console.log(this.article.contentHtml)
				if (updateAbout.data.status == 1) {
					this.$message.success('提交成功');
				} else {
					this.$message.error(updateAbout.data.info);
				}
			}
		},
		async mounted() {
