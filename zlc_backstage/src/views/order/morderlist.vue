						...
						<div class="goods_item" v-for="(item,i) in scope.row.info" :key="i" :class="i==scope.row.info.length-1?'border0':''">
							...
							<div class="cont_ite">
								...
								<div class="lable">
									...
									<span>¥{{item.price}} x{{item.num}}件 </span>
								</div>
							</div>
						</div>
					</template>
					...
					<template slot-scope="scope">
						<el-dropdown trigger="click" split-button size="mini" type="primary">
							...
							<el-dropdown-menu slot="dropdown">
								...
								<el-dropdown-item v-if="scope.row.status>=3" style="text-align: center;">
									<el-button size="mini" @click="id=scope.row.order_id,showlog=true" style="margin: 5px 0;">查看物流</el-button>
								</el-dropdown-item>
								<el-dropdown-item v-if="scope.row.status==2" style="text-align: center;">
