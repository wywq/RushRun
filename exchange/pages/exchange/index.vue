    ...
    <view class="exchange-body">
      ...
      <view class="exchange-value">
        <view class="exchange-value-column">
          ...
          <view class="exchange-value-item">
            ...
            <view class="exchange-value-item-value"
              >￥{{ response.zuidi }}</view
            >
          </view>
          <!-- 最高 -->
        ...
        <view class="exchange-value-column">
          ...
          <view class="exchange-value-item">
            ...
            <view class="exchange-value-item-value">{{ response.maidan }}</view>
          </view>
          <!-- 涨跌 -->
          <view class="exchange-value-item">
            <view class="exchange-value-item-title">涨跌</view>
            <view class="exchange-value-item-value"
              >{{ response.zhangdie }}%</view
            >
          </view>
        </view>
      </view>
      ...
      <view class="exchange-title">
        <template v-if="activeTab == 1">
          <view class="exchange-title-one">买家</view>
        </template>
        ...
        <block v-for="(i, index) in dataList" :key="index">
          <view class="exchange-list-item">
            <view class="exchange-item-one">
              <image class="exchange-item-one-icon" :src="i.head_pic"></image>
              <view class="exchange-item-one-name">{{ i.nickname }}</view>
            </view>
            <view class="exchange-item-two">{{ i.num }}</view>
            <view class="exchange-item-two">{{ i.price }}</view>
            <view class="exchange-item-three">
              <template v-if="activeTab == 1">
                <view
                  class="exchange-item-btn-yellow"
                  @tap="handleSell(i.order_id)"
                  >出售</view
                >
              </template>
              <template v-if="activeTab == 2">
                <view
                  class="exchange-item-btn-blue"
                  @tap="handleBuy(i.order_id)"
                  >购买</view
                >
              </template>
            </view>
...
<script>
import { todayInfo, needList, sellList, buy, sell } from "@/api/new.js";
...
export default {
  ...
  methods: {
    ...
    onHandleRight() {
      uni.navigateTo({
        url: "/pages/exchange/publishChoose/index",
      });
    },
    onViewClick(options) {
      console.log(options);
