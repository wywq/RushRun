...
<script>
	export default {
		...
		methods: {
			...
			async edit() {
				if(this.form.old_psw == '' || this.form.new_psw == '' || this.form.password2 == '')
				{
					this.$message({
						message:'请补全信息',
						type:'warning'
				...
				let postdata = {
					pwd_old: this.form.old_psw,
					pwd_new: this.form.new_psw,
					pwd_2:this.form.password2,
					admin_id:JSON.parse(sessionStorage.getItem("admin")).admin_id
				}
				let res = await this.$zcy.updateAdmin(postdata)
